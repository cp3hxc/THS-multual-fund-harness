...
**列表查询优先**：列表接口已经覆盖展示所需的全部字段（受理时间、类型、基金、金额/份额、订单号、预计确认等）。常规「展示今日/近期交易记录」场景只跑 `aijijin trade list` 即可，**不要无差别对每条记录循环调用详情接口**。

只有以下业务场景才需要按 `appSheetSerialNo` 用 `aijijin trade detail` 补全：

- `confirmFlag=1` 表示已撤单，`confirmFlag=4` 表示确认失败，`confirmFlag=6` 表示交易作废；这三类记录**永远不要查详情**。
- `subBusinessType` 包含 `reservation` 或描述包含“认购”时必须查询详情，以处理部分确认与 `refundAmount`。
- 基金转换（`firstBusinessType=change` 或 `subBusinessType=normal_change`）记录必须查询详情，以获取确认转出/转入份额和净值。
- `subBusinessType=redemption_for_buying` 时必须查询详情，以获取 `targetFundCode/targetFundName`。
- 买入记录（`firstBusinessType=buy`）做资金来源分析或到账时间追踪时，按需查询详情；纯展示则不必查。
- 卖出记录（`firstBusinessType=sell`）**默认不查详情**——列表已含 `totalFundUnits`，详情也不会回填卖出金额（按份额发起、确认时按净值计算）；等到记录确认（`confirmFlag=3`）后再查详情补 `confirmedAmount` 用于已实现盈亏汇总。

### 详情补全后的字段处理规则

仅在按上述规则确需调用详情时，以下规则才生效：
...
4. 判断是否需要补全 → 按「列表字段补全规则」逐条筛选：仅对 reservation/转换/redemption_for_buying/买入待确认等命中场景调用详情；常规展示直接跳过
5. 详情补全（按需）→ aijijin trade detail --order-id <订单号>
